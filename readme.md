...
**性能测试**：`v2/tests/PerformanceTest.cpp`。先做预热，再在相同负载下对比内存池 `allocate/deallocate` 与系统 `new`/`delete` 的耗时（基于 `std::chrono`）。
...
| 场景说明                     | 主要参数与负载说明                          | 内存池耗时 | 系统 new/delete 耗时 | 相对关系   |
|------------------------------|---------------------------------------------|------------|----------------------|------------|
| 小对象单线程                 | 50000 次分配，固定小尺寸 8/16/32/64/128/256 B | ~4.13 ms   | ~3.37 ms             | 池略慢     |
| 多线程分配                   | 4 线程，每线程 25000 次分配/释放            | ~10.43 ms  | ~12.16 ms            | 池约快 14% |
| 混合大小单线程               | 100000 次分配，多种固定小尺寸               | ~3.74 ms   | ~6.36 ms             | 池约快 41% |

v2 在**多线程**与**混合大小、单线程**场景下均表现出优于系统默认分配器的趋势，与 ThreadCache 减少锁争用、按大小类批量管理等设计相符；单线程纯小对象场景下略慢，可能与测试规模与本地缓存行为有关。
...
- **v2**：单测全部通过；在多线程与混合大小分配场景下，相对系统 new/delete 有可见优势（约 14%、41%），适合在简历或文档中概括为「多线程与混合大小场景下性能优于系统分配器」。
